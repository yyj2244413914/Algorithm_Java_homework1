...
public class ComparaSort {
     ...
     */
    public static Double[] getSortedRepeatedData(int N, double repeatedRange) {
    Double[] data = GenerateData.getSortedData(N);  // 生成正序数组
        ...
        for (double repeatedRange : repeatRates) {
            ...
            for (int lenIdx = 0; lenIdx < dataLength.length; lenIdx++) {
                ...
                for (int groupIdx = 0; groupIdx < T; groupIdx++) {
                    testDataGroups[groupIdx] = getSortedRepeatedData(n, repeatedRange);
                }
